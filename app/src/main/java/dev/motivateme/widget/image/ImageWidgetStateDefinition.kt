...
object ImageWidgetStateDefinition : GlanceStateDefinition<ImageWidgetState> {

    private const val DATA_STORE_FILENAME_PREFIX = "image_widget_state_"
    ...
    // Custom serializer
    object WidgetStateSerializer : Serializer<ImageWidgetState> {
        override val defaultValue = ImageWidgetState.Unavailable("Image not found")

        override suspend fun readFrom(input: InputStream): ImageWidgetState = try {
            Json.decodeFromString(
                ImageWidgetState.serializer(),
        ...
        override suspend fun writeTo(t: ImageWidgetState, output: OutputStream) {
            output.use {
                it.write(
                    Json.encodeToString(ImageWidgetState.serializer(), t).encodeToByteArray()
                )
            }
        }
    }
